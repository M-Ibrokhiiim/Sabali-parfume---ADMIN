...
const TRANSLATIONS = {
  uz: {
    ...
    availableWarehouse: 'Omborda maroon',
    ...
    dbCacheDesc: 'Standart namunalarni tiklash va LocalStorage-ni toзалash',
...
export const useStore = () => {
  ...
  // Sync and save state changes directly to LocalStorage (Lightning Fast!)
  ...
  // Fetch from LocalStorage (Lightning Fast!)
      ...
      if (isBrowser) {
        ...
        if (savedProducts) {
          try {
            ...
            products.value = [...DEFAULT_PRODUCTS]
            await syncAndSave(DEFAULT_PRODUCTS)
          }
        } else {
          products.value = [...DEFAULT_PRODUCTS]
          await syncAndSave(DEFAULT_PRODUCTS)
  ...
  const addProduct = async (productData: any) => {
    ...
    const newProduct: Product = {
      ...
      volume: productData.volume || '100ml',
      category: productData.category || 'Men',
      stock: Number(productData.stock ?? 10),
      description: productData.description || '',
      images: productData.images || [], // base64 arrays are preserved!
      sales: 0,
      rating: 5.0,
      createdAt: new Date().toISOString(),
      starred: productData.starred || false
    }
  ...
  // Update Product (Fully Local!)
  const updateProduct = async (id: string, updatedData: any, silent = false) => {
    if (!silent) {
      loading.value = true
    }
    error.value = null
